...
function Account() {
  ...
  const handleAvatarChange = (e) => {
    e.preventDefault();
    setCurrentAvatarUrl(
      avatars.find((avatar) => avatar.avatar_id === selectedAvatar)?.img_url
    );
    changeAvatar();
    setShow(false);
    setTimeout(() => {
      ...
      <div className="row">
        <div className="col-4">
          <div className="p-4 bg-light d-flex justify-content-center flex-column w-100 align-items-center shadow-sm">
            ...
            <button
              className="btn  btn-fantasy text-white"
          ...
          <div className="p-4 rounded bg-light shadow-sm">
            ...
            {games.length === 0 ? (
              <>
                ...
                <div className="mt-4">
                  <p>
                    <i>Begin your journey right now...</i>
                  </p>
                  <Link to="/story">
                    <button
                      type="button"
              ...
              <>
                ...
                {games.map((el) => (
                  <div className="row border-bottom py-3" key={el.game_id}>
                    ...
                    <span className="col-6 d-flex align-items-center justify-content-start">
                      {new Date(el.date_played).toISOString().split('T')[0]}
                    </span>
                    <span className="col-4 d-flex align-items-center justify-content-center">
                      {el.game_status === true
                        ? 'Win'
                        : el.game_status === false
                        ? 'Lost'
                        : 'Draw'}
                    </span>
                  </div>
